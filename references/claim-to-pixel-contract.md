...
已验证事实可用 `titleTokens` 精确声明它能支持标题或 promise 中的高风险词。例如事实文本确实包含“3 种画幅”，才可列出 `"3种画幅"`。脚本会同时核对普通文本和重新抽取的“数字 + 单位”，因此事实里的 `110倍` 不能靠子串伪装成登记 token `10倍`；公开文案验证也同样精确。`3种画幅` 可以认证其中的 `3种`，绝对词只按抽取后的显式短语匹配。

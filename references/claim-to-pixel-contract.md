...
已验证事实可用 `titleTokens` 精确声明它能支持的高风险词。例如事实文本确实包含“3 种画幅”，才可列出 `"3种画幅"`。脚本会阻止不在事实文本中的 token。数字与单位必须精确一致，`110倍` 不能认证 `10倍`；绝对词只允许已知父短语的受控包含关系，例如显式登记的 `全网唯一` 可以同时覆盖其中的 `唯一`。

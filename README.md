...
如果当前工作区存在 `CRENSTUDIO/`，它是统一交付根目录。每个任务创建一个日期项目目录，将最终视频放入 `video/`、字幕和转写放入 `subtitles/`、封面放入 `covers/`，并在同一项目保留 `source/`、`review/`、提示词和 `generation.json`。不要把成品直接散落在工作区根目录。

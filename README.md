...
未约定平台时默认 **3:4 + 9:16**；“横竖都要”增加 **16:9**；公众号仍输出 **21:9 + 1:1**。原仓库与杂志风格使用可编辑HTML，其余六种使用内置生图并保留完整提示词。图片实际尺寸会核验记录，不把目标尺寸或放大结果当成原生高清。
...
请先核对事实，视觉重点放在真人与主证据，人物按主题构图，方图单独排版。
```
...
- 3:4 与 21:9 根据视觉重点和内容路由安排人物，不再固定左下；
- 优先保留脸、手、话筒、桌面和动作上下文；
- 优先使用完整环境照片，避免粗糙抠图；
- 1:1 默认短标题优先，以保证微信列表缩略图可读；
- 如果 1:1 需要人物，会重新构图并同时检查脸和标题，而不是裁切横版。
...
微信列表显示很小，所以方图先保证 4–10 字标题可读。若这次视觉重点是小伟真人、现场复盘或人物与产品的关系，方图可以放人物，但会重新设计人物占比、标题和留白，不会从横版裁切。

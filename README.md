...
未约定平台时按公域社媒处理，默认 **3:4 + 9:16**；“横竖都要”增加 **16:9**；只有明确要求公众号时才输出 **21:9 + 1:1**。小伟真人封面默认真人必须出镜，姿势、动作、位置、景别和占比按选题变化。原仓库与杂志风格使用可编辑HTML，其余六种使用内置生图并保留完整提示词。图片实际尺寸会核验记录，不把目标尺寸或放大结果当成原生高清。
